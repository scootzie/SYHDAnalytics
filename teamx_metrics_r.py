...
WITH openAndPull1Day AS (
  SELECT createdAt::DATE AS "date", COUNT(*) FILTER(WHERE name='Open App') AS openAppCountDay, COUNT(*) FILTER(WHERE action='view connection list' AND method='pull up tab bar') AS pullUpTabBarCountDay
  ...
  WHERE name='Open App' OR (action='view connection list' AND method='pull up tab bar')
  GROUP BY 1 
)
SELECT date, openAppCountDay, pullUpTabBarCountDay, 
            ...
            (SELECT count(*)
            FROM Event JOIN Interaction ON Event.interactionID=Interaction.ID JOIN InteractionType ON Interaction.interactiontypeID=InteractionType.ID
            WHERE action='view connection list' AND method='pull up tab bar' AND createdAt::DATE BETWEEN openAndPull1Day.date - 7 AND openAndPull1Day.date) 
            ...
            (SELECT count(memberid)
            FROM Event JOIN Interaction ON Event.interactionID=Interaction.ID JOIN InteractionType ON Interaction.interactiontypeID=InteractionType.ID
            WHERE action='view connection list' AND method='pull up tab bar' AND createdAt::DATE BETWEEN openAndPull1Day.date - 29 AND openAndPull1Day.date) 
